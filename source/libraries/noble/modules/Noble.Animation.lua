...
function Noble.Animation.new(__view)
	...
	-- The current seconds we have been on the frame. This is used to determine when to advance to the next frame.
	animation.frameDurationCount = 0
	...
	-- @int[opt=1] __frameDuration This is the number of seconds between each frame in this animation. If not specified, it will be set to 1/30.
	-- @usage
	...
	function animation:addState(__name, __startFrame, __endFrame, __next, __loop, __onComplete, __frameDuration, __frameDurationRandom)

		local loop = true
		local frameDuration = 1/30
		if (__loop ~= nil) then loop = __loop end
		if(__frameDuration ~= nil) then frameDuration = __frameDuration end
		__frameDurationRandom = __frameDurationRandom or 0
		self[__name] = {
			name = __name,
			...
			frameDurationRandom = __frameDurationRandom
		}
		...
		if (empty == true) then
			...
			self.frameDuration = self.calculateFrameDuration(self.current)
		end
	...
	function animation.calculateFrameDuration(state)
		return state.frameDuration + (math.random() * state.frameDurationRandom)
	end
	...
	function animation:update()
		...
		local elapsedTime = Noble.elapsedTime
		self.frameDurationCount += elapsedTime
		if self.frameDurationCount >= self.frameDuration then

			self.currentFrame = self.currentFrame + 1
			-- Subtract frame duration to roll over any remaining time.
			self.frameDurationCount -= self.frameDuration
			-- Recalculate the frameDuration as it might change due to random.
			self.frameDuration = self.calculateFrameDuration(self.current)

			if(self.currentFrame == self.current.endFrame + 1) then	-- End frame behavior.
	...
	--	end
	function animation:draw(__x, __y)
		local x = __x or 0
		local y = __y or 0
