...
function NobleSprite:update()
	if (self.animation ~= nil) then
		local changed = self.animation:update()
		if changed then
			self:markDirty()
		end
	end
...
function NobleSprite:draw(__x, __y)
	if (self.animation ~= nil) then
		-- Always draw the anim at 0, 0 as it needs to draw the whole thing.
...
function NobleSprite:setAnimation(animation)
	local currentName = nil
	local currentFrame = 1
	local frameDurationCount = 0
	if self.animation then
		currentName = self.animation.currentName
		currentFrame = self.animation.currentFrame		
		frameDurationCount = self.animation.frameDurationCount
	end
	...
	if currentName then
		self.animation:setState(currentName)
		self.animation.currentFrame = currentFrame
		self.animation.frameDurationCount = frameDurationCount
	end

	self:setSize(animation.imageTable[1]:getSize())
	self:markDirty()
end

function NobleSprite:setAnimationImageTable(imageTable)
	if not self.animation then
		error("Trying to set animation imageTable but animation has not been created")
		return
	end
	
	self.animation.imageTable = imageTable
	self:setSize(imageTable[1]:getSize())
	self:markDirty()
end